            ...
            <div class="container-fluid py-1 px-2">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-1 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Edit Kriteria 2</li>
                    </ol>
                </nav>
            </div>
        </nav>
                ...
                <div class="col-12">
                    <div class="card border shadow-xs mb-4">
                        <div class="custom-form-section p-4">
                            <h4 class="form-title text-right mb-4">Edit Kriteria 2 - Tata Kelola, Tata Pamong , dan
                                Kerjasama</h4>
                            ...
                            @php
                                $fields = [
                                    ...
                                    [
                                        'name' => 'pelaksanaan',
                                        'label' => 'Pelaksanaan',
                                        'id' => 'pendukung_pelaksanaan',
                                    ],
                                    ['name' => 'evaluasi', 'label' => 'Evaluasi', 'id' => 'pendukung_evaluasi'],
                                    [
                                        'name' => 'pengendalian',
                                        'label' => 'Pengendalian',
                                        'id' => 'pendukung_pengendalian',
                                    ],
                                    [
                                        'name' => 'peningkatan',
                                        'label' => 'Peningkatan',
                                        'id' => 'pendukung_peningkatan',
                                    ],
                                ];
                            @endphp

                            <form method="POST" action="{{ route('kriteria2.update', $detail->id_detail_kriteria) }}"
                                    ...
                                    <div class="row mb-4">
                                        <div class="col-md-9">
                                            <label for="{{ $field['name'] }}" class="form-label"
                                                style="font-size: large; color: #1e293b">
                                                {{ $field['label'] }}
                                            </label>
                                            <textarea name="{{ $field['name'] }}" id="{{ $field['name'] }}"
                                                class="form-control @error($field['name']) is-invalid @enderror"
                                                placeholder="Masukkan {{ strtolower($field['label']) }}">{{ old($field['name'], $detail->{$field['name']}->deskripsi ?? '') }}</textarea>
                                            @error($field['name'])
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="col-md-3">
                                            <label for="{{ $field['id'] }}" class="form-label">Pendukung
                                                {{ $field['label'] }} (Upload File)</label>
                                            <div class="file-input-wrapper position-relative">
                                                ...
                                                <label for="{{ $field['id'] }}" class="file-input-label">Choose a
                                                    file</label>

                                                <div id="filePreview{{ $index + 1 }}" class="file-preview">
                                                    <button type="button" id="closeButton{{ $index + 1 }}"
                                                    ...
                                                    <div id="previewBox{{ $index + 1 }}" class="preview-box"
                                                        style="width: 350px; height: 300px; border: 2px dashed #ccc; display: flex; align-items: center; justify-content: center;">
                                                        <span id="plusSign{{ $index + 1 }}" class="plus-sign"
                                                            style="font-size: 48px; color: #ccc;">+</span>
                                                    </div>
                                                    <p id="fileName{{ $index + 1 }}"></p>
                                                    <img id="imagePreview{{ $index + 1 }}" src="#"
                                                        alt="Image Preview"
                                                        style="display: none; max-width: 350px; max-height: 300px; object-fit: contain; margin: 0 auto;" />
                                                    <i id="pdfIcon{{ $index + 1 }}" class="fa fa-file-pdf-o"
                                                        style="display: none; font-size: 80px; color: #d9534f;"></i>
                                                </div>
                                                ...
                                                @if (isset($detail->{$field['name']}->pendukung) && $detail->{$field['name']}->pendukung)
                                                    ...
                                                    @php
                                                        ...
                                                        $storagePath = str_starts_with($filePath, 'public/')
                                                            ? substr($filePath, 7)
                                                            : $filePath;
                                                    ...
                                                    <div class="mt-2">
                                                        @if (in_array($ext, ['jpg', 'jpeg', 'png', 'gif']))
                                                            <img src="{{ $fileUrl }}" alt="Preview lama"
                                                        ...
                                                        @else
                                                            <a href="{{ $fileUrl }}" target="_blank">Lihat File
                                                                Pendukung Lama</a>
                                                        @endif
                                                    </div>
                                                @endif
                                ...
                                <div class="mt-4 text-end">
                                    ...
                                    <a href="{{ route('kriteria2.index') }}"
                                        class="btn btn-danger px-4 me-2">Cancel</a>
                                </div>
                            </form>
                        </div>
                    </div>
