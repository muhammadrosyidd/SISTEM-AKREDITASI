...
<body class="g-sidenav-show bg-gray-100">
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body px-0 pt-0 pb-2">
                            <div class="table-responsive p-3">
                                <table class="table align-items-center mb-0" id="table_detail_kriteria" style="width:100%">
                                    <thead>
                                        <tr>
                                            <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7" width="10%">No</th>
                                            <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7" width="40%">Nama Batch</th>
                                            <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7" width="20%">Tanggal</th>
                                            <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 text-center" width="15%">Status</th>
                                            <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 text-center" width="15%">Aksi</th>
                                        </tr>
                                    </thead>
                                    <tbody></tbody>
                                </table>
    ...
    <script>
        const base_url = "{{ url('validasiDirektur') }}";

        function showDetail(id_pengisian) {
    ...
    var modalEl = document.getElementById('modal-master');
    var modal = new bootstrap.Modal(modalEl);
    modal.show();

    $.ajax({
        url: "{{ route('validasi.show') }}",
        method: "POST",
        data: {
            id: id_pengisian,
            _token: "{{ csrf_token() }}"
        ...
        error: function() {
            $('#modal-content-placeholder').html(`
                ...
                <div class="modal-body">
                    <div class="alert alert-danger">Gagal memuat data validasi. Silakan coba lagi.</div>
                </div>
            `);
        }
    });
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row, meta) {
                            const id = row.id_pengisian;
                            return `
                                <div class="btn-action-group d-flex justify-content-center">
                                    <button class="btn btn-info btn-sm" onclick="showDetail(${id})" title="Validasi">
                                        <i class="fas fa-eye fa-xs"></i> Validasi
                                    </button>
                                </div>
                            `;
                        }
